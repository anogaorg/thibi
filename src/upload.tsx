    ...
    return (
        <>
            <form onSubmit={handleSubmit((e) => submitHandler(e, props, sqlite))}>
                <div>
                    <label htmlFor={id} className='block text-gray-700 text-sm font-bold mb-2'>Choose a CSV</label>
                    ...
                    />
                    <p id='file_upload_help' className='mt-1 text-sm text-gray-500 dark:text-gray-300'>Upload a file to start an analysis job for Thibi</p>
                </div>

                <input className="btn-primary text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type='submit' value='Upload File'></input>
            </form>
        </>
    );
}
...
function UploadPreview(props: PreviewProps) {
    ...
    const renderJobs = () => {
        return props.uploadDone.map((data) => {
            return (
                <JobsTable key={data[0]} data={data}></JobsTable>
            );
        });
    };
    ...
    return (
        props.uploadDone ?
            ...
            : <button type="button" className="btn-primary bg-lime-600 inline-flex items-center px-4 py-2 font-semibold leading-6 text-sm shadow rounded-md text-white hover:bg-lime-400 transition ease-in-out duration-150 cursor-not-allowed" disabled>
